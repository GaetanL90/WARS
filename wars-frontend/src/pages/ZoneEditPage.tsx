...
export function ZoneEditPage() {
  ...
  useEffect(() => {
    if (id) {
      const existingZone = zoneStore.getById(id);
      if (existingZone) {
        setZoneName(existingZone.name);
        setZoneNotes(existingZone.notes || "");
        setSelectedVillages(existingZone.villages || []);
      }
    }
  }, [id]);
  ...
  const handleUpdateZone = (e: FormEvent) => {
    e.preventDefault();
    if (!zoneName.trim() || selectedVillages.length < 2 || !id) return;

    zoneStore.update(id, {
      name: zoneName,
      sector: selectedVillages[0]?.sector || "",
    ...
    navigate(`/dashboard/infrastructure/zones/${id}`);
  };
    ...
    <div className="page-container">
      <div className="page-header">
        <div className="breadcrumbs mb-8">
          ...
          <Link to={`/dashboard/infrastructure/zones/${id}`}>Details</Link> /{" "}
          <span>Edit</span>
        </div>
        <div>
          <h1>Edit Distribution Zone</h1>
          <p style={{ margin: '4px 0 0', color: '#64748b' }}>
            Update the parameters and coverage of this water zone.
          </p>
        </div>
      </div>
      ...
      <div className="grid-2 flex-gap" style={{ marginTop: '24px', alignItems: 'start' }}>
        ...
        <div className="card">
          ...
          <h3 style={{ fontSize: '1.1rem', marginBottom: '16px', paddingBottom: '12px', borderBottom: '1px solid #e2e8f0' }}>Manage Villages</h3>
          <p style={{ fontSize: '0.85rem', color: '#64748b', marginBottom: '16px' }}>
            Zone region is strictly locked to <strong>Eastern Province, Bugesera District</strong>.
          </p>
        ...
        <div>
          ...
          <div className="card" style={{ background: '#f8fafc' }}>
            <h4 style={{ fontSize: '0.9rem', marginBottom: '12px', color: '#334155' }}>Validation Status</h4>
            ...
            <div className="flex-gap" style={{ gap: '12px' }}>
              <button className="btn btn-ghost flex-1" onClick={() => navigate(-1)}>Cancel</button>
              <button 
